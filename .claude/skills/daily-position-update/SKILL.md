...
- `current_price_krw`는 사람이 손대는 칸이 아니다 — `collect.py`가 시세 조회 시
  자동 갱신한다. 건드리지 말 것.
- **신용주식 매도가 있었다면 그 계좌의 결과 예수금을 반드시 사용자에게 직접
  물어본다** — 매도대금으로 역산하지 않는다 (`CLAUDE.md`의 "신용주식 매도 시
  예수금은 반드시 직접 확인" 참고, 실제로 역산 오차로 다음날 전일비가 어긋난
  사례가 있었다). 신용 없는 일반 매수·매도의 예수금 변동은 기존처럼 매매대금
  역산도 허용된다.

수정 후 diff를 사용자에게 보여주고 확인받는다 (숫자 파일이라 조용히 틀리면 바로
안 보인다).

### 매매 사유 Nudge (Lookback 매매일지용) — 무조건 역질문

이번 업데이트에 신규매수·추가매수·매도(현금/신용 종목 모두, ETF 포함) 거래가
하나라도 있었는데 **사용자가 그 거래의 사유·심리 상태를 먼저 말해주지 않았다면**,
숫자 반영과 별개로 **반드시 그 자리에서 역질문을 던진다** — "왜 이 매매를
했는지" 한 줄이라도 남겨달라고. 사용자가 매매 요약을 줄 때 수량·가격만 있고
이유가 없는 게 기본 패턴이므로, 매번 빠짐없이 물어봐야 한다(생략 금지).

- 거래별로 전부 물을 필요는 없다 — 오늘 있었던 거래들을 묶어서 한 번에
  "오늘 매매 이유(간단히)는?"처럼 짧게 물으면 된다.
- 사용자가 "간단히"라고 답해도 괜찮다 — 목적은 완결된 일지가 아니라 `journal.csv`의
  `memo`/`lesson` 칸이 공란으로 남지 않게 하는 것.
- 답을 받으면 `data/journal.csv`에 해당 거래 행으로 추가한다 (칼럼:
  `date,account,name,action,qty,price,amount,planned,memo,trigger,emotion,violated_rules,realized_pnl,lesson,entry_via`).
  `entry_via`는 `chat`.
- 사용자가 질문에 답하지 않고 다음 지시로 넘어가면 더 조르지 않는다 — 한 번
  물었으면 충분하고, journal.csv는 공란으로 둔다(추정 채움 금지).
